...
Pollard's rho algorithm (PR) \cite{pollard} uses two increasing values modulo $N$ to generate numbers with which to find the greatest common divisor with $N$.
The method uses the sequence $x_{i+1} = x^2 + 1 \bmod N$, where $i>0$, to generate two values, $x_i$ and $x_{2i}$.
The difference is taken and a check for a common divisor with $N$ larger than one is performed.
\\

This is better than arbitrarily testing random numbers modulo $N$.
Firstly since if $N$ is composite, at least one divisor $d$ has to be less than or equal to $\sqrt{N}$.
blablablabla står en del i citerade man kanske kan nämna\cite{csh}

Some improvements where made to the algorithm by Richard Brent and Pollard.\cite{brent}
...
This can however introduce problems if the product $q$ grows to $q = 0 \bmod N$, which leads to finding the factor $d=N$.
However, this can be countered by not multiplying too many times, and to backtrack to the last $x_i$ and $x_{2i}$ and performing the regular pollard rho procedure if a factor $d=N$ is discovered.
...
Different values of $a$ are tried, hoping that the expression $a^2 - N$, which is equivalent to $b^2$, produces a square.
