...
% Förvirringen ligger i att inte blanda ihop val av två slumpade med koll om den sökta diffen ens finns inom sekvensen.
% Annan pdf som går igenom sambandet mellan birthday och pollards:
% http://www.mpi-inf.mpg.de/~csaha/lectures/lec18.pdf 
% Röstar för den här genomgångsordningen: birthday -> sekvens -> floyd's cycle detection trick -> pollard's
% pdfen ovan gör ungefär så, fast sekvens innan birthday. Gillar det.
...
% skrev om meningen under till <= sqrt.
If we choose values $x_i, x_j \leq \sqrt{N}$, there are $2(\sqrt{N}-d)$ different combinations of $x_i$ and $x_j$ that has the property $\abs{x_i - x_j} = d$. % Insåg att deras påstående gäller diffen, så abs. Dubbelkollade med wolfram.
...
% Insert finishing sentence below. The above probability is higher than the one for 1/sqrt when d < sqrt/2.
% In other words, picking two random numbers is better when d < sqrt/2
% Om jag tänker rätt så är det svaret för två random, men för en sekvens av k förbättras det, så om vi förklarar
% sekvensanvändningen ovan borde vi kunna visa att det alltid är bättre i pollard.

% Nedanstående måste ändras för att matcha ovanstående.
...
% Man kan skriva vilken generator som används och flytta övriga delen till diskussion om man vill.
% Tyckte att det var okej att ta upp här dock. Men, fundera på saken.
...
These produce a congruence of squares.% EXEMPEL ????
...
An exponent vector is a vector in the form $(4, 2)$, where the elements signify the prime exponents for the $i$-th prime.
...
\bibitem{floyd} Floyd's cycle-finding algorithm - \url{http://en.wikipedia.org/wiki/Cycle_detection#Tortoise_and_hare}
