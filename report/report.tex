...
Pollard's rho algorithm, including Brent's improvement, was implemented using pair programming.\\
Fermat's factorization method was implemented by Gustaf.\\
Optimization of Brent's factorization method was done by Martin.
...
Pollard's rho algorithm \cite{algnotes,colorado} is a probabilistic algorithm that tries to find one factor of a composite number.
...
Pollard's rho algorithm therefore repeatedly calculates two values $x_i$ and $x_2i$ from the sequence and calculates $gcd(\abs{x_{2i}-x_i}, N)$.
...
The algorithms were written in |C| and the GMP \cite{gmp} library was used to represent large numbers and perform functions such as primality testing and GCD computation.
...
When using Pollard's Rho algorithm and the improved Brent version only the first 600-1000 primes were used for trial division.
Both the regular Pollard's algorithm and the Brent version were dynamically limited on the number of |gcd| calculations made, based on the size of the integer being factored.
...
For Fermat's algorithm, we used the first 9000 primes.
For Pollard's and Brent's we used the 1000 and 600 first primes respectively.

Pollard's rho algorithm got results between 57 and 73, and Brent's factorization method got results between 73 and 87.
In order to avoid going over the time limit of 15 seconds we had to set a limit on the number of iterations that the algorithms made.
...
We also had to skip numbers above a certain size.
Basic Pollard's rho got its best results when we only tried to factorize numbers smaller than 86 bits.
...
One of the few times $C=3$ did not lead to \emph{Time Limit Exceeded} in Kattis it was 2 seconds slower than for $C = 1$.

According to Brent, the value of $m$ should often be low, and in general $\log{N} \ll m \ll N^{1/4}$ as mentioned earlier.
...
Trial division and primality testing was enough to factorize some factors, which wasn't surprising since we expected that a few numbers in the problem set would be intentionally easy.

Fermat's method managed to factorize a few numbers but was too slow to be effective.
The fact that the optimal configuration for the fermat implementation used the first 9000 primes for trial-division, compared to the 600-1000 used by the Pollard/Brent algorithms, further hinted at the slowness of the algorithm.

Pollard's Rho algorithm gave much better results than Fermat's, most likely due to successfully utilizing the birthday paradox.
...
We are not sure why setting the value of $C$ to 3 as suggested by Brent slowed down the program compared to setting it to 1 as in the original Pollard's algorithm. However, considering the fact that we only tried to factor a very limited amount of numbers, this difference could be circumstantial.
It might also have been due to poor interaction with other variables, or some arcane difference in hardware.

The fact that some theory was not applicable could have been due to bad luck with the problem set.
For example, setting a dynamic $m$ never improved our results, but in theory it could have.
This could also be because we misinterpreted the theory.
...
We saw how it could potentially be really fast by simplifying the factor finding to mere XOR operations.
However, after reading up on how to implement it we came to the conclusion that it would probably take a lot of time to implement and debug and that the time we had left would not suffice.
