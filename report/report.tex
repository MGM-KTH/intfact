...
%  OBS: Vi måste förklara eller inte nämna sekvensen. 
...
% In other words, picking two random numbers <= sqrt is better when d is < sqrt/2
...
The pseudo-random generator $f(x)$ is defined as: $x^2+C\, (mod\, N)$, where $C$ is an arbitrary constant which maintains the property\\ $x = y\, (mod\; p) \rightarrow f(x) = f(y)\,(mod\; p)$.\\
Pollard suggested $C = 1$, but there are many other polynomials that satisfy this requirement.\\ % Source? Än så länge wikipedia.
